...
export default function GrantedBeneficiaryAccessList() {
  ...
  if (error) return <div className="bg-secondary border border-default rounded-2xl p-4 h-96 overflow-auto customScrollbar">Error : {JSON.stringify(error)}</div>;
  if (!data || data.getAllGrantedBeneficiaries.length === 0)
    return (
      <DevOnly>
        <div className="bg-secondary border border-default rounded-2xl p-4 h-96 overflow-auto customScrollbar text-sm">
    ...
    <div className="bg-secondary border border-default rounded-2xl p-4 h-96 overflow-auto customScrollbar flex flex-col gap-4">
      ...
      {data?.getAllGrantedBeneficiaries.map((access) => {
        return (
          <div
            key={access.id}
            className="flex items-center justify-between bg-white dark:bg-neutral-800 border border-default rounded-2xl p-2"
          >
            <div>
              <div>
