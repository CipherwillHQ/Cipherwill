...
export default function MetaDetails({ id }: { id: string }) {
  ...
  if (error && 'errors' in error) {
    ...
    if (
      ...
      window.location.href = "/app/data/passwords";
      return null;
    }
  }
  ...
  return (
    <div className="flex flex-col sm:flex-row items-center gap-2 justify-between py-2">
        ...
        <div
          ...
          onBlur={(e) => {
            ...
            if (new_name.length === 0) {
              const nameElement = document.getElementById("password-name");
              if (nameElement) {
          ...
          onKeyDown={(e) => {
            if (e.key === "Enter") {
              e.preventDefault();
              e.stopPropagation();
              document.getElementById("password-name")?.blur();
            }
          }}
        >
          {parsedData.name}
      ...
      <div className="flex items-center gap-2">
        <button
          onClick={() => {
            document.getElementById("password-name")?.focus();
          }}
        >
          Rename
        </button>
