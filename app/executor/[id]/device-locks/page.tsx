...
export default function GrantedDeviceLocks() {
  ...
  return (
    <div className="w-full">
      <h1 className="text-xl font-semibold">Device Locks</h1>
      <div className="flex flex-col gap-2" data-cy="donor-models">
        {final_models.length === 0 && (
          <div className="py-2 opacity-50">No Device Locks Found</div>
        )}
        {final_models.map((model: GrantedMetamodel) => {
          ...
          return (
            <Link
              key={model.id}
              href={`/executor/${id}/device-locks/${model.id}`}
